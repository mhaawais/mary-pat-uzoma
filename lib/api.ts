...
async function fetchContent<T extends { id: string; title: string; slug?: string }>(
  type: string,
  fallback: T[],
  process: (item: T, slugs: Set<string>) => T
): Promise<T[]> {
  try {
    const response = await fetch(`${API_BASE_URL}/api/sync?type=${type}`, {
    ...
    return items.map((item: T) => process(item, existingSlugs));
  } catch (error) {
    console.error(`Error fetching ${type}:`, error);
    const existingSlugs = new Set<string>();
    return fallback.map((item) => {
      if (!item.slug) {
        item.slug = generateSlug(item.title, item.id, existingSlugs);
      } else {
...
export async function fetchBooks(): Promise<Book[]> {
  return fetchContent<Book>("books", fallbackBooks, (book, slugs) => {
    if (!book.slug) {
      book.slug = generateSlug(book.title, book.id, slugs);
    } else {
      slugs.add(book.slug);
    }
    return book;
...
export async function fetchBlogs(): Promise<Blog[]> {
  return fetchContent<Blog>("blogs", fallbackBlogs, (blog, slugs) => {
    if (!blog.slug) {
      blog.slug = generateSlug(blog.title, blog.id, slugs);
    } else {
      slugs.add(blog.slug);
    }
    return blog;
...
export async function fetchProfile(): Promise<Profile | null> {
  try {
    const response = await fetch(`${API_BASE_URL}/api/sync?type=profile`, {
      headers: { Authorization: `Bearer ${API_KEY}` },
      next: { revalidate: 3600 },
    });
