...
const Footer = ({ books = [], profile }: FooterProps) => {
  ...
  const toggleBooks = () => {
    if (isBooksOpen) {
      ...
      setIsBooksOpen(true);
      if (closeTimer.current) clearTimeout(closeTimer.current);
      closeTimer.current = setTimeout(() => setIsBooksOpen(false), 60000);
    }
  };
    ...
    <footer className="text-gray-600 body-font bg-black py-10 md:py-16">
      <div className="container px-5 md:px-14 lg:px-20 mx-auto flex flex-col items-center">
        ...
        >
          ...
          <Link href="/author" className="hover:text-myred w-full text-center md:w-auto">AUTHOR</Link>
          <Link href="/blog" className="hover:text-myred w-full text-center md:w-auto">BLOG</Link>
          <Link href="/contactus" className="hover:text-myred w-full text-center md:w-auto">CONTACT US</Link>
        </nav>
