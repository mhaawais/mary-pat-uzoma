...
const Header = ({ books = [] }: HeaderProps) => {
  ...
  const openDropdown = () => {
    ...
    if (timeoutRef.current) clearTimeout(timeoutRef.current)
    timeoutRef.current = setTimeout(() => setBookDropdown(false), 60000)
  }
          ...
          >
            <ul className="flex flex-col md:flex-row items-start md:items-center md:space-x-10 lg:space-x-16 py-4 md:py-0 px-6 md:px-0">
              <li className="relative w-full md:w-auto group" onMouseEnter={handleMouseEnter}>
                ...
                <ul
                  className={`transform transition-all duration-300 ease-in-out md:absolute left-0 mt-2 bg-black md:shadow-lg shadow-md rounded-sm w-full md:w-64 z-50 border border-white border-solid ${
                    bookDropdown ? "block scale-100 opacity-100" : "hidden scale-95 opacity-0"
                  }`}
                >
